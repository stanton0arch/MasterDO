 ...
 * calculated constants of vdp.h and taken in DRAM at init. One buffer is
 * enough while the cel engine is drawn from a picture that no renderer is
 * rewriting underneath it; whether more than one is worth its 36 kilobytes
 * is a decision that waits for the draw measurements, and this constant is
 * where that decision will be applied when it is taken.
 *
