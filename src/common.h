...
/*
 * How the picture is drawn, in every build. The background is an image
 * of 256 by 224 pixels, one byte each -- the whole name table -- that
 * the video part keeps in step tile by tile, at the tiles the program
 * rewrote, and that the cel engine draws by windows of that one image:
 * a scroll join, a locked row or column, a band of the picture drawn
 * with an older state of the video memory, each is a window with its
 * own source pointer, size and position and the same row pitch. The
 * sprites and the priority tiles go through the same list, in every
 * band, after its windows: a sprite is a window of a sheet of converted
 * sprite patterns, the highest numbered first so that the lowest shows
 * on top, cut to the lines the hardware admits it on; a priority tile
 * is a window of the background image under a second palette whose
 * colour 0 of either bank is transparent; the lines with the display
 * off and the masked left column are cels of a backdrop column, last.
 * No pixel is written by the processor. The two sprite bits of the
 * status register are computed per line, without a pixel, from a table
 * built when the sprite attribute table changes -- the overflow read
 * off it, the collision replayed on the sprites that share a column.
 * The picture is presented when line 191 has been counted, before the
 * blanking lines, because the writes of those lines belong to the next
 * picture; the cels land inside a clip rectangle set on each screen at
 * init, which is what erases the up to three columns a window has to
 * read before its first pixel to start on a word.
 *
 ...
 *
 * Three switch names of the road to this are refused below, so that an
 * old build command fails loudly rather than silently building the
 * default. The first chose between this list and a render where the
 * processor composed every pixel of every line into an index buffer of
 * 48 kilobytes that one cel drew whole; the list has drawn the reference
 * ROM line for line as that render did (tests/cel8/, the frozen
 * references), and the render, its buffer and its page left the build.
 * The second broke that render down into posts by running one of them
 * twice per line; it went with the render. The third counted the index
 * buffers; there is none.
 */
#ifdef SMS_DECOR_CEL
#error "SMS_DECOR_CEL no longer exists: the cel list is the one way the picture is drawn, the per-pixel render left the build"
#endif

#ifdef SMS_VDP_PROFILE
#error "SMS_VDP_PROFILE no longer exists: the per-pixel render it broke down left the build"
#endif

#ifdef SMS_VDP_BUFFERS
#error "SMS_VDP_BUFFERS no longer exists: there is no index buffer, the cel engine draws the picture from the list"
#endif
 ...
 * as they stood at its first line: one band from line 0, then one from
 * each distinct line the journal holds a write on.
 ...
 *   memory probe the development build plus SMS_MEM_PROBE 1. It is not
 *                another program but the same one with an instrument
 *                bolted on, and NOTHING ABOUT ITS SPEED MAY BE QUOTED: it
 *                takes 128 kilobytes the shipped build does not, 64 of
 *                them out of the VRAM the display owns, and it stalls the
 *                boot for some twenty seconds before the first frame.
 *                What it publishes is a cost per memory access, which is
 *                a property of the machine and not of this build.
 *
 *   cel probe    the development build plus SMS_CEL_PROBE 1. The same
 *                instrument-bolted-on kind as the one above, and further
 ...
 *
 * In every one of them but the cel probe the frame loop executes the
 * cartridge by scanline quotas, the video part takes its share of one
 * line per quota, and the presentation -- the list built and drawn band
 * by band when line 191 has been counted -- closes the picture: there is
 * one executor and one path, and the switches above decide only what is
 * traced and measured along it. The cel probe is the one exception: it
 * stops the boot before the loop and holds its pattern. In every build
 * the emulated palette is the screen's own colour table, set on each
 * screen once at the start of the loop and then at most once per frame,
 * after a colour moved, while the cels' palette is the identity; no
 * switch turns that off, since nothing else in the program can turn an
 * index into a colour.
 *
 * Two more switch names are refused below, for the reason the three
 * above are. The built-in test suite once had a second way to run, from
 * the boot sequence one
 ...
 * Refused loudly, as every switch that prints is.
 */
